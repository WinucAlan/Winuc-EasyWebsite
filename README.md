...
```
或者在`exe_process.txt`里添加外部exe处理列表，具体格式如下：
```
识别方式（完整识别用A，前缀识别用P） 网站路径（/xxx，主页面为/，可以叠加） 处理的exe路径
```
> 外部程序处理时，发送的命令为 `处理的exe路径 请求方IP 请求内容路径`
...
5. `exe_process.txt`里网站路径（/xxx，主页面为/，可以叠加）、处理的exe路径不能有空格！
